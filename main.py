...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS journal (
        ...
        resultat TEXT, pnl REAL, commentaire TEXT, created_at TEXT
    )''')
    conn.commit()
    conn.close()
...
def telegram():
    ...
    r = requests.post(
        ...
        json={"chat_id": TELEGRAM_CHAT_ID, "text": data.get("text",""), "parse_mode": "HTML"}
    )
...
def add_trade():
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('''INSERT INTO journal (date,pair,tf,session,score,decision,bias,entry,sl,tp,rr,resultat,pnl,commentaire,created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
        (data.get("date"),data.get("pair"),data.get("tf"),data.get("session"),data.get("score"),
         ...
         data.get("rr"),data.get("resultat"),data.get("pnl"),data.get("commentaire"),datetime.now().isoformat()))
    conn.commit()
...
def update_trade(trade_id):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("UPDATE journal SET resultat=?,pnl=?,commentaire=? WHERE id=?",
        (data.get("resultat"),data.get("pnl"),data.get("commentaire"),trade_id))
    conn.commit()
    conn.close()
    return jsonify({"success": True})
